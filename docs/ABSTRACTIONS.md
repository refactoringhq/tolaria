...
Standalone `.html` and `.htm` vault entries load as ordinary text tabs, but default to `HtmlFilePreview` instead of BlockNote or CodeMirror. `htmlFilePreviewSrcDoc` sanitizes the whole document, installs a restrictive Content Security Policy, removes remote passive loads, rewrites only vault-contained local resource paths to Tauri asset URLs, and appends a static reveal stylesheet (`data-tolaria-static-preview`) that counters script-gated visibility (`opacity:0`/`visibility:hidden`), fullscreen pinning, and document scroll locks so decks authored for JS slideshow runners stay readable without script execution. Mermaid sources inside `pre.mermaid`, `div.mermaid`, or `code.language-mermaid` are rendered to static SVG by the shared `mermaidRender` host runtime (`renderSrcDocMermaid`) before the iframe mounts, with unparseable diagrams degrading to their source text. The preview iframe has an opaque origin and no script or same-origin sandbox capability, so its document cannot access Tolaria's DOM, storage, or Tauri IPC. The existing raw-source preference is shared with Markdown: the breadcrumb source action or `Cmd/Ctrl+\` swaps the preview for CodeMirror, and the external-open command remains the escape hatch for browser-only behavior. The breadcrumb derives action availability from the entry kind: standalone HTML keeps source/preview, file location, link, PDF export, git diff, and delete actions while omitting Markdown metadata, neighborhood, reading-width, table-of-contents, and archive controls.
...
- `editorRichCopy.ts` owns rich-editor copy serialization for external apps. Normal selections use BlockNote's external clipboard HTML so tables, lists, checklists, and inline marks paste as rich content outside Tolaria, while `SingleEditorView` still normalizes `text/plain` and keeps fenced code-block selections on raw code text. Wikilink selections stay on the rich HTML path: `writeRichEditorClipboardPayload` rewrites each wikilink element into its `[[target]]` literal via `htmlWithWikilinkLiterals` and adds a `text/markdown` flavor, instead of flattening the whole selection to plain markup, so one link no longer strips formatting from everything around it.
- `editorFullNoteCopy.ts` owns whole-note clipboard export and context-menu paste. `fullNoteClipboardPayload` serializes the active document through BlockNote's lossy external exporter and restores wikilink labels to `[[target]]` in both HTML and Markdown, `writeFullNoteClipboard` writes `text/html` + `text/plain` through the Web Clipboard API and falls back to the native text bridge when WebKit rejects a gesture-less write, and `pasteClipboardIntoEditor` prefers clipboard HTML via `editor.pasteHTML` over `text/plain` via `editor.pasteText` with `requestPlainTextPaste` as the final fallback. `copyFullNoteToClipboard` emits `editor_copy_full_note` with the achieved mode.
- `EditorContextMenu.tsx` provides the rich-editor right-click menu (copy selection, paste, select all, copy full note) on the `SingleEditorView` container. The native WebKit context menu is suppressed globally in `main.tsx`, so the hook reuses the note-list portal/positioning pattern (`contextMenuPosition.ts`), preserves the DOM selection until the copy action runs via `onMouseDown` prevention, and hides copy when no selection lives inside the editor. Menu open/action/paste-mode outcomes go to PostHog.
- `useCopyFullNoteCommand.ts` registers the editor-side copy-full-note bridge into `copyFullNoteRef` (the same ref pattern as find-in-note) so the `editCopyFullNote` manifest command reaches the BlockNote editor from the native Edit menu and the command palette's "Copy full note" entry.
