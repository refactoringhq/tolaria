...
- `createRichEditorEmptyListNavigationExtension()` keeps persisted empty bullet, checklist, numbered-list, and toggle-list items reachable by plain vertical arrows. It also prevents BlockNote's second Backspace on a nested empty paragraph from lifting list descendants or siblings: a parent with children stays in place, while a childless empty paragraph alone is removed. Explicit Tab/Shift+Tab remains the only path for changing list depth. The extension otherwise leaves wrapped lines, modified arrow selection, IME composition, and code-block navigation on their existing paths.
