...
- The Edit menu's "Copy Full Note" and the command palette's "Copy full note" share the `editCopyFullNote` manifest command; `Editor.tsx` registers the active BlockNote editor into `copyFullNoteRef` following the find-in-note ref pattern, and the rich-editor context menu's "Copy Full Note" item calls the same editor-side implementation directly
