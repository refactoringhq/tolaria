...
import { UI_LANGUAGE_OPTIONS, normalizeUiLocale, t as translateMessage, useI18n } from '../lib/i18n'
import {
  useState,
  useRef,
  useCallback,
...
interface SettingsBodyProps {
  uiLanguage: UiLocale
  setUiLanguage: (value: UiLocale) => void
  pullInterval: number
...
function createSettingsDraft(
  ...
  return {
    uiLanguage: normalizeUiLocale(settings.ui_language) ?? 'zh-CN',
...
>) {
  ...
  return (
    <>
      <SectionHeading
        title={t('AutoGit')}
        description={t(autoGitSectionDescription(isGitVault))}
      />
...
function buildDefaultAiAgentOptions(aiAgentsStatus: AiAgentsStatus): Array<{ value: string; label: string }> {
  return AI_AGENT_DEFINITIONS.map((definition) => {
    ...
    const suffix = status.status === 'installed'
      ? ` (${translateMessage('installed')}${status.version ? ` ${status.version}` : ''})`
      : ` (${translateMessage('missing')})`
    return {
      value: definition.id,
      label: `${translateMessage(definition.label)}${suffix}`,
    }
  })
}
...
function renderDefaultAiAgentSummary(defaultAiAgent: AiAgentId, aiAgentsStatus: AiAgentsStatus): string {
  ...
  if (status.status === 'installed') {
    return status.version
      ? translateMessage('{label} {version} is ready to use.', { label: translateMessage(definition.label), version: status.version })
      : translateMessage('{label} is ready to use.', { label: translateMessage(definition.label) })
  }
  return translateMessage('{label} is not installed yet. You can still select it now and install it later.', { label: translateMessage(definition.label) })
}

function LanguageSection({
  uiLanguage,
  setUiLanguage,
}: Pick<SettingsBodyProps, 'uiLanguage' | 'setUiLanguage'>) {
  const { t } = useI18n()
  ...
  return (
    <>
      <SectionHeading
        title={t('Language')}
        description={t('Choose the interface language used across Tolaria.')}
      />

      <LabeledSelect
        label={t('Interface language')}
        value={uiLanguage}
        onValueChange={(value) => setUiLanguage(value as UiLocale)}
...
function OrganizationWorkflowSection({
  ...
  return (
    <>
      <SectionHeading
        title={t('Workflow')}
        description={t('Choose whether Tolaria shows the Inbox workflow and the organized toggle.')}
      />
