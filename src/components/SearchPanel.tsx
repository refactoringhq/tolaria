  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <SearchContent
          searchHint={t('Search across all note contents')}
          keyboardHint={t('Enter to open · Esc to close')}
          searchingLabel={t('Searching...')}
          noResultsLabel={t('No results found')}
          resultCountLabel={(count, elapsed) => elapsed === null
            ? t(count === 1 ? 'resultsNoTime' : 'resultsNoTime_plural', { count })
            : t(count === 1 ? 'results' : 'results_plural', { count, elapsed })}
          query={query}
          results={results}
...
const SearchInput = forwardRef<HTMLInputElement, SearchInputProps>(
  function SearchInput({ query, loading, placeholder, onChange, onKeyDown }, ref) {
    return (
...
interface SearchContentProps {
  ...
  resultCountLabel: (count: number, elapsed: number | null) => string
}

function SearchContent({
  query, results, selectedIndex, loading, elapsedMs, entryLookup, typeEntryMap, listRef, onSelect, onHover,
  searchHint, keyboardHint, searchingLabel, noResultsLabel, resultCountLabel,
}: SearchContentProps) {
  return (
    <div className="flex-1 overflow-y-auto">
      {!query.trim() && (
        <div className="px-4 py-8 text-center">
          <p className="text-[13px] text-muted-foreground">{searchHint}</p>
      ...
      {query.trim() && results.length === 0 && !loading && (
        <div className="px-4 py-8 text-center">
          <p className="text-[13px] text-muted-foreground">{noResultsLabel}</p>
        </div>
      )}
        ...
        <>
          <div className="border-b border-border/50 px-4 py-1.5">
            <span className="text-[11px] text-muted-foreground">
              {resultCountLabel(results.length, elapsedMs)}
            </span>
          </div>
          <div ref={listRef}>
